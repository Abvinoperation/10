...
@app.route('/admin/attendance/force_checkout/<int:att_id>', methods=['POST'])
def force_checkout(att_id):
    if not is_admin_logged_in():
        return redirect(url_for('login'))
    ...
    try:
        ...
        cur.execute("SELECT * FROM attendance WHERE att_id = %s", (att_id,))
        ...
        if attendance['check_out'] is not None:
            flash('Employee already checked out', 'warning')
            return redirect(url_for('manage_attendance', date=date_str))
        
        # Calculate work hours
        check_in = attendance['check_in'].strftime('%Y-%m-%d %H:%M:%S')
        check_out_datetime = f"{date_str} {check_out_time}"
        total_hours = calculate_work_hours(check_in, check_out_datetime)
        ...
        cur.execute("""
            ...
            WHERE att_id = %s
        """, (check_out_datetime, total_hours, att_id))
        mysql.connection.commit()
        flash('Forced check-out recorded successfully!', 'success')
    except Exception as e:
        mysql.connection.rollback()
        flash(f'Error forcing check-out: {str(e)}', 'danger')
    finally:
        cur.close()
